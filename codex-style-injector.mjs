...
Examples:
  codex-style-injector.mjs --css ~/Development/codex-themes/codex-theme.css --port 9333
  codex-style-injector.mjs --css ~/Development/codex-themes/codex-theme.css --once --port 9333
`);
}
...
function buildInjectionSource(cssText, jsText) {
  return `(() => {
...
function buildSourceBundle(opts) {
  ...
  const source = buildInjectionSource(cssText, jsText);
...
function createFileWatchers(filePaths, onChange) {
  ...
  for (const absPath of filePaths) {
    ...
    try {
      ...
      console.error(`[hot-reload] failed to watch ${absPath}: ${err instanceof Error ? err.message : String(err)}`);
      continue;
    }

    watcher.on('error', (err) => {
      console.error(`[hot-reload] watcher error for ${absPath}: ${err instanceof Error ? err.message : String(err)}`);
    });
...
async function main() {
  ...
  let bundle = buildSourceBundle(opts);
  ...
  const watchedPaths = [bundle.cssPath, bundle.jsPath].filter(Boolean);
  const watchers = [];
  ...
  const tick = async (reason = 'poll') => {
    ...
    try {
      ...
      for (const target of pages) {
        ...
        try {
          const meta = await injectTarget(target, bundle.source);
          injectedHashByTargetId.set(target.id, bundle.hash);
          console.log(`[inject] id=${target.id} hash=${bundle.hash} reason=${reason} url=${target.url}`);
          if (meta) {
            console.log(`         appliedAt=${meta.appliedAt} cssBytes=${meta.cssBytes} jsBytes=${meta.jsBytes}`);
          }
        } catch (err) {
          console.error(`[inject] failed id=${target.id} url=${target.url}: ${err instanceof Error ? err.message : String(err)}`);
        }
      }

      if (pages.length === 0) {
        if (!noTargetsWarned) {
          console.warn('[watch] waiting for matching targets...');
      ...
      if (opts.once && pages.length === 0) {
        console.warn('[inject] no matching page targets found');
      }
    } finally {
      tickRunning = false;
  ...
  const reloadBundleFromDisk = async (triggerPath) => {
    let nextBundle;
    try {
      nextBundle = buildSourceBundle(opts);
    } catch (err) {
      console.error(`[hot-reload] failed to read source: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }
    ...
    bundle = nextBundle;
    console.log(`[hot-reload] source updated hash=${bundle.hash} trigger=${triggerPath}`);
  ...
  const scheduleReload = (triggerPath) => {
    ...
    reloadTimer = setTimeout(() => {
      reloadBundleFromDisk(triggerPath).catch((err) => {
        console.error(`[hot-reload] reload failed: ${err instanceof Error ? err.message : String(err)}`);
      });
    }, 150);
  };
  ...
  console.log(`[watch] host=${opts.host} port=${opts.port} targetPrefix=${opts.targetUrlPrefix} intervalMs=${opts.intervalMs} hash=${bundle.hash}`);
  ...
  if (opts.hotReload && watchedPaths.length > 0) {
    ...
    watchers.push(...createFileWatchers(watchedPaths, (absPath, eventType) => {
      console.log(`[hot-reload] fs event=${eventType} path=${absPath}`);
  ...
  const interval = setInterval(() => {
    tick('poll').catch((err) => {
      console.error(`[watch] tick failed: ${err instanceof Error ? err.message : String(err)}`);
    });
  }, opts.intervalMs);
